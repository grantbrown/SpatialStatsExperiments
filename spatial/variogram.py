...
class GaussianVariogram(Variogram):
    ...
    def f(self, t):
        return((t > 0) * (self.tau_squared + self.sigma_squared*(1-np.exp(-1*(self.phi**2)*(t**2)))))
        #if (t <= 0.0):
        #    return(0.0
        #return(self.tau_squared + self.sigma_squared*(1-np.exp(-1*(self.phi**2)*(t**2))))

    def C(self, t):
        return(((t <= 0) * (self.tau_squared + self.sigma_squared))
             + ((t > 0) * (self.sigma_squared * np.exp((-1) * (self.phi**2)*(t**2)))))
